...
router.post('/generarReporte', (req, res) => {
    const { fechaInicio, fechaFin, sucursalBuscar, tipoReporte } = req.body;
    ...
    // LÃ­mite mÃ¡ximo de registros permitidos
    ...
    let countQuery = `
        ...
        WHERE Fecha >= ? 
          ...
          AND IdProveedor = ?
    `;
    let queryParams = [fechaInicio, fechaFin, idProveedor];
    ...
    connection.query(countQuery, queryParams, (err, countResults) => {
        ...
        if (totalRecords > MAX_RECORDS) {
            // Si los registros exceden el lÃ­mite, devolver un mensaje de error
        ...
        if (tipoReporte === 'agrupado') {
            query = `
                SELECT 
                    ...
                    NombreSucursal, 
                    Upc, 
                    DATE_FORMAT(MAX(Fecha), '%Y-%m-%d') as Fecha,
        ...
        connection.query(query, queryParams, (err, results) => {
            ...
            reportCacheVentas[req.session.IdUsuarioP] = results;
...
router.get('/exportarTodo', (req, res) => {
    ...
    const dataToExport = reportCacheVentas[userId].map(item => {
    ...
    const fields = ["NombreSucursal", "Upc", "Descripcion", "Cantidad", "MontoTotal", "Departamento", "Categoria", "SubCategoria", "UnidadesPorFardo", "Fecha"];

    try {
        const csv = parse(dataToExport, { fields }); // Usa json2csv para crear el archivo CSV

        // Crear el nombre del archivo con las fechas
        const fileName = `ReporteVentasGeneral_${inicio}_${fin}.csv`;
...
router.post('/exportarFiltrado', (req, res) => {
    const { sucursal, departamento, categoria, subcategoria, searchTerm, fechaInicio, fechaFin } = req.body;
    const userId = req.session.IdUsuarioP;
