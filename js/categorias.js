document.addEventListener('DOMContentLoaded', function() { 
    ...
    function fetchSucursales() {
        showSpinner();
        fetch(`${basePath}backendCategorias/obtenerSucursales`)
    ...
    function fetchCategorias() {
        showSpinner();
        fetch(`${basePath}backendCategorias/obtenerCategorias`)
        .then(response => response.json())
        .then(data => {
        ...
        .finally(() => {
            hideSpinner(); // Se ejecuta siempre
        });;      
    }
    fetchCategorias();
    ...
    function generarReporte(tipoReporte) {
        ...
        const subcategoriaBuscar = document.getElementById('categoriaBusqueda').value; // Obtener el valor de la subcategorÃ­a seleccionada
        ...
        const url = `${basePath}backendCategorias/generarReporte`;
        const requestBody = {
            fechaInicio,
            fechaFin,
            sucursalBuscar: sucursalBuscar === '' ? null : sucursalBuscar,
            subcategoriaBuscar: subcategoriaBuscar === '' ? null : subcategoriaBuscar, // Incluir subcategorÃ­a en el request
            tipoReporte // Este serÃ¡ 'agrupado' o 'detallado'
        };
        ...
        .then(result => {
            ...
            updateTable(result.totalRecords, tipoReporte, sucursalBuscar, subcategoriaBuscar, fechaInicio, fechaFin);
    ...
    function updateTable(totalRecords, tipoReporte, sucursalBuscar, subcategoriaBuscar, fechaInicio, fechaFin) {
        ...
        $('#dataTable').DataTable({
            ...
            ajax: function (data, callback) {
                ...
                const url = `${basePath}backendCategorias/getPaginatedData`;
                const requestBody = {
                    page,
                    pageSize,
    ...
    $('#dataTable tbody').on('click', '.btn-nombre-sucursal', function () {
        ...
        fetch(`${basePath}backendCategorias/detalleSucursal?nombre=${encodeURIComponent(id)}`)
            .then(response => {
                if (!response.ok) {
    ...
    function actualizarInformacion() {
        ...
        fetch(`${basePath}backendCategorias/filtrarDatos`, {
            method: 'POST',
    ...
    function verButtonFiltro() {
        // No limpiar el input de bÃºsqueda
    ...
    function obtenerTotales() {
        ...
        fetch(`${basePath}backendCategorias/calcularTotales`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ sucursal, departamento, categoria, subcategoria, proveedor, searchTerm }) // Incluir filtros aplicados
        })
        .then(response => {
            if (!response.ok) {
    ...
    function generarGraficas() {
        ...
        google.charts.setOnLoadCallback(() => {
            ...
            crearGraficoBar('ventasPorProveedor', Object.keys(proveedorTotales), Object.values(proveedorTotales), `Gráfica por Proveedor`);
        });
    }
    ...
    document.getElementById('exportar').addEventListener('click', function () {
        ...
        const url = `${basePath}backendCategorias/exportarTodo?inicio=${encodeURIComponent(inicio)}&fin=${encodeURIComponent(fin)}`;
    ...
    document.getElementById('exportFilter').addEventListener('click', function () {
        ...
        const categoria = document.getElementById('categoriaFiltro').value || null;
        const subcategoria = document.getElementById('subcategoriaFiltro').value || null;
        const provedor = document.getElementById('proveedorFiltro').value || null;
        const searchTerm = document.getElementById('descProduc').value.trim() || null;
        ...
        form.method = 'POST';
        form.action = `${basePath}backendCategorias/exportarFiltrado`;
    
        const fields = { sucursal, departamento, categoria, subcategoria, provedor, searchTerm, fechaInicio, fechaFin };
